...
2. Template logic
    ...
    ```njk
        {% macro renderCard(options) %}
          ...
          {% set url = options.url | default(defaults.url) | default('#') %}
          {% set title = options.title | default(defaults.headingName) | default('Sans titre') %}
          {% set excerpt = options.excerpt | default(defaults.excerpt) | default('') %}
...
**Priority**: `options.X | default(defaults.X) | default(fallback)`

## Rationale

Priority explicit in code. Validation prevents silent failures. Backward compatible with defaults-only usage. Aligns with Eleventy Data Cascade.

## Consequences

Applies to components displaying CMS content (ADR-012 Tier 1-2): cards, sections, editorial content containers.

Manual `| default()` chains required. Tier 3 restrictions must be enforced. Validation code repeated per component.
