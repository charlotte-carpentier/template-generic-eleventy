...
const CONFIG = {
    DROPZONE_SELECTOR: '[data-drag-drop]',
  ...
  export function initBlockDragAndDrop() {
    const dropzones = document.querySelectorAll(CONFIG.DROPZONE_SELECTOR);
    dropzones.forEach(dropzone => setupDropzone(dropzone));
  }
  ...
  function setupDropzone(dropzone) {
    const input = dropzone.querySelector(CONFIG.INPUT_SELECTOR);
    if (!input) return;
    
    // Get configuration from data attributes
    const maxSize = parseInt(dropzone.getAttribute('data-max-size')) || CONFIG.MAX_FILE_SIZE;
    const allowedTypes = dropzone.getAttribute('data-allowed-types')?.split(',') || CONFIG.ALLOWED_TYPES;
    const multiple = dropzone.hasAttribute('data-multiple');
    ...
    dropzone.addEventListener('drop', (e) => {
      ...
      handleFiles(files, dropzone, input, maxSize, allowedTypes, multiple);
    }, false);
    ...
    input.addEventListener('change', (e) => {
      handleFiles(e.target.files, dropzone, input, maxSize, allowedTypes, multiple);
    });
